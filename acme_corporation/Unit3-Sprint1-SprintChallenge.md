...
I find the most important part of code review to be readability. When writing code on my own it is easy to overestimate the comprehensibility of my code. Having another person look at the code and understand how it works is the gold standard for comprehensibility. The first thing I looked at in my code review was the docstrings, and if they are inadequate there is little chance I will be able to quickly grasp the point of the code. Visual clarity, (under PEP8) is another importand part of code readability.

Containers are helpful because they allow us to perfectly reproduce an environment without going to all the trouble and computing expense to set up virtual machines everywhere. A single small image file contains all the information you need to make a container, which will be the same everywhere, regardless of the OS or other specifications of the computer it is being run on. By contrast, a virtual machine takes a great deal of computing power and can be time-consuming to set up. Image files can be including along with the code with minimal fuss and can include any necessary prerequisites for the code. This becomes increasingly helpful as a codebase increases in size and complexity, with a large number of modules that depend on each other.
### Part 8 - Turn it in!
